...
#f1 = get_news_list1(url4,'//td[@height="32"]/text()')
#f2 = get_news_list1(url4,'//td[@width="100"]/text()')
#get_news_list(url2,'.//td[@width="367"]')
#get_news_list(url)
...
#vim.current.buffer.append(['--'*20,''])
#vim.current.buffer.append(['--'*20,'公安厅网络传真','--'*20])
#vim.current.buffer.append([i+' '+j for (i,j) in zip(f1,f2)])
#vim.current.buffer.append(['--'*20,''])
